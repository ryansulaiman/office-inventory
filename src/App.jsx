...
// Unit prefix map
const UNIT_PREFIXES = { phones:"PHN", laptops:"LAP", printers:"PRN" };
...
export default function App() {
  ...
  useEffect(() => {
    loadAll();
    const channels = [
      supabase.channel("users-ch").on("postgres_changes", { event:"*", schema:"public", table:"users" }, loadAll),
      supabase.channel("inventory-ch").on("postgres_changes", { event:"*", schema:"public", table:"inventory" }, loadAll),
      supabase.channel("assignments-ch").on("postgres_changes", { event:"*", schema:"public", table:"assignments" }, loadAll),
      supabase.channel("requests-ch").on("postgres_changes", { event:"*", schema:"public", table:"requests" }, loadAll),
      supabase.channel("transfers-ch").on("postgres_changes", { event:"*", schema:"public", table:"transfers" }, loadAll),
      supabase.channel("incidents-ch").on("postgres_changes", { event:"*", schema:"public", table:"incidents" }, loadAll),
      supabase.channel("logs-ch").on("postgres_changes", { event:"*", schema:"public", table:"logs" }, loadAll),
      supabase.channel("units-ch").on("postgres_changes", { event:"*", schema:"public", table:"units" }, loadAll),
      supabase.channel("unit-assignments-ch").on("postgres_changes", { event:"*", schema:"public", table:"unit_assignments" }, loadAll),
    ];
  ...
  const assignUnit = async () => {
    const { requestId, unitId } = form;
    if (!unitId) return showToast("Please select a unit", "error");
    const req = requests.find(r => r.id === Number(requestId));
    const unit = getUnit(Number(unitId));
    if (!unit || unit.status !== "available") return showToast("Unit is not available", "error");
    // Mark unit as assigned
    await supabase.from("units").update({ status: "assigned" }).eq("id", unit.id);
    // Create unit assignment
    await supabase.from("unit_assignments").insert({ unit_id: unit.id, user_id: req.from_user_id, assigned_at: today, status: "active" });
    // Approve the request
    await supabase.from("requests").update({ status: "approved", assigned_unit_id: unit.id }).eq("id", req.id);
    await addLog("Unit Assigned", currentUser.name, `Assigned ${unit.unit_code} to ${getUser(req.from_user_id)?.name}`);
    setModal(null); setForm({}); showToast(`${unit.unit_code} assigned!`);
  };
  ...
  const submitRequest = async () => {
    const { itemId, note } = form;
    if (!itemId) return showToast("Please select an item", "error");
    const item = getItem(Number(itemId));
    await supabase.from("requests").insert({ from_user_id: currentUser.id, item_id: Number(itemId), qty: 1, status: "pending", note: note || "", date: today });
    await addLog("Request", currentUser.name, `Requested ${item.name}`);
    ...
    return (
      <div style={{ minHeight:"100vh", background:BRAND.gradient, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
        <div style={{ background:"white", borderRadius:24, padding:"32px 28px", width:"100%", maxWidth:340, boxShadow:"0 25px 60px rgba(0,0,0,0.35)", textAlign:"center", position:"relative" }}>
          ...
          <div style={{ fontSize:13, color:"#9ca3af", marginBottom:24 }}><RoleBadge role={selectedUser?.role} /> · Enter your PIN</div>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"16px 12px 90px" }}>
        ...
        {tab === "dashboard" && isAssistant && (
          <div>
            ...
            {pendingRequests.length > 0 && (
              <div style={{ background:"#ede9fe", border:`1.5px solid ${BRAND.light}`, borderRadius:14, padding:14 }}>
                <div style={{ fontWeight:700, color:BRAND.dark, fontSize:14, marginBottom:8 }}>📋 {pendingRequests.length} Pending Request{pendingRequests.length>1?"s":""} — needs your action</div>
                {pendingRequests.slice(0,3).map(req => <div key={req.id} style={{ fontSize:13, color:BRAND.darker, marginBottom:4 }}>• {getUser(req.from_user_id)?.name} needs {getItem(req.item_id)?.name}</div>)}
              </div>
            )}
          </div>
        )}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"flex-start", gap:8 }}>
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                        {isTracked && <button onClick={() => { setModal("generateUnits"); setForm({ itemId:String(item.id), prefix: Object.entries(UNIT_PREFIXES).find(([,v])=>item.name.toLowerCase().includes(Object.keys(UNIT_PREFIXES).find(k=>UNIT_PREFIXES[k]===v))||false)?.[1] || "" }); }} style={{ background:"#dbeafe", color:"#1d4ed8", border:"none", borderRadius:7, padding:"5px 9px", fontSize:11, cursor:"pointer", fontWeight:700 }}>+ Units</button>}
                        {!isTracked && <button onClick={() => { setModal("generateUnits"); setForm({ itemId:String(item.id) }); }} style={{ background:"#dbeafe", color:"#1d4ed8", border:"none", borderRadius:7, padding:"5px 9px", fontSize:11, cursor:"pointer", fontWeight:700 }}>Track</button>}
                        ...
                        <button onClick={() => setConfirmAction({ type:"deleteItem", itemId:item.id, name:item.name })} style={{ background:"#fee2e2", color:"#ef4444", border:"none", borderRadius:7, padding:"5px 9px", fontSize:12, cursor:"pointer", fontWeight:700 }}>🗑️</button>
                      </div>
                    </div>
                  </div>
                  {isTracked && (
        ...
        {tab === "devices" && isAdminOrAssistant && (
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              ...
              {isAdmin && <button onClick={() => { setModal("generateUnits"); setForm({}); }} style={{ background:BRAND.primary, color:"white", border:"none", borderRadius:10, padding:"8px 14px", fontSize:13, cursor:"pointer", fontWeight:700 }}>+ Add Units</button>}
            </div>
            ...
            {inventory.filter(item => units.some(u => u.item_id === item.id)).map(item => {
              ...
              return (
                <div key={item.id}>
                  <SectionLabel label={`${item.name} — ${itemUnits.length} units`} />
                  {itemUnits.map(unit => {
                    const activeUA = unitAssignments.find(ua => ua.unit_id === unit.id && ua.status === "active");
            ...
            {units.length === 0 && <div style={{ color:"#9ca3af", textAlign:"center", marginTop:60 }}>No tracked units yet. Add units from the Inventory tab.</div>}
          </div>
        )}
              ...
              return (
                <div key={status}>
                  ...
                  {group.map(req => {
                    ...
                    const itemUnits = units.filter(u => u.item_id === req.item_id && u.status === "available");
                    const isTrackedItem = units.some(u => u.item_id === req.item_id);
                    const assignedUnit = req.assigned_unit_id ? getUnit(req.assigned_unit_id) : null;
                    return (
                      <Card key={req.id}>
                        <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                          ...
                          <div style={{ flex:1 }}>
                            ...
                            <div style={{ fontSize:13, color:"#374151", marginTop:2 }}>Requesting <strong>{item?.name}</strong></div>
                            {req.note && <div style={{ fontSize:12, color:"#6b7280", fontStyle:"italic" }}>"{req.note}"</div>}
                            {assignedUnit && <div style={{ fontSize:13, color:BRAND.primary, fontWeight:700, marginTop:4 }}>📱 Assigned: {assignedUnit.unit_code}</div>}
                        ...
                        {req.status === "pending" && (
                          <div style={{ marginTop:12 }}>
                            {isTrackedItem ? (
                              <>
                                <div style={{ fontSize:12, fontWeight:600, color:"#6b7280", marginBottom:6 }}>Select unit to assign:</div>
                                <select style={inp} value={form[`unit_${req.id}`] || ""} onChange={e => setForm(f => ({ ...f, [`unit_${req.id}`]:e.target.value }))}>
                                  <option value="">Choose a unit...</option>
                                  {itemUnits.map(u => <option key={u.id} value={u.id}>{u.unit_code} — {u.condition}</option>)}
                                </select>
                                <div style={{ display:"flex", gap:8, marginTop:10 }}>
                                  <button onClick={() => { if (!form[`unit_${req.id}`]) return showToast("Please select a unit", "error"); setForm(f => ({ ...f, requestId:String(req.id), unitId:f[`unit_${req.id}`] })); assignUnit(); }} style={{ flex:1, background:BRAND.primary, color:"white", border:"none", borderRadius:8, padding:"9px", cursor:"pointer", fontWeight:700, fontSize:13 }}>✓ Assign & Approve</button>
        ...
        {tab === "log" && isAdmin && (
          <div>
            ...
            {logs.map(l => {
              const iconMap = { "Approved":"✅","Rejected":"❌","Transfer Sent":"📤","Transfer Accepted":"✅","Transfer Declined":"❌","Return":"↩️","Added Item":"➕","Edited Item":"✏️","Deleted Item":"🗑️","User Added":"👤","User Removed":"🗑️","Request":"📋","Damaged":"🔧","Lost":"❓","Incident Resolved":"✅","PIN Changed":"🔑","Unit Assigned":"📱","Unit Returned":"↩️","Unit Damaged":"🔧","Unit Lost":"❓","Units Generated":"📦" };
              return (
              ...
              return (
                <Card key={ua.id} style={{ borderLeft:`4px solid ${BRAND.primary}` }}>
                  ...
                  <div style={{ display:"flex", gap:6, marginTop:12 }}>
                    <button onClick={() => { setModal("transfer"); setForm({ unitId:String(unit.id) }); }} style={{ flex:1, background:BRAND.pale, color:BRAND.dark, border:"none", borderRadius:9, padding:"8px", cursor:"pointer", fontWeight:700, fontSize:12 }}>🔄 Transfer</button>
                    <button onClick={() => { setModal("reportUnitIncident"); setForm({ unitId:String(unit.id), reportedBy:currentUser.name }); }} style={{ flex:1, background:"#fff7ed", color:"#c2410c", border:"none", borderRadius:9, padding:"8px", cursor:"pointer", fontWeight:700, fontSize:12 }}>⚠️ Report</button>
                  </div>
                </Card>
              );
            })}
              ...
              {requests.filter(r => r.from_user_id === currentUser.id).map(req => {
                ...
                return (
                  <Card key={req.id}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div>
                        <div style={{ fontWeight:700, fontSize:14 }}>{getItem(req.item_id)?.name}</div>
                        {assignedUnit && <div style={{ fontSize:13, color:BRAND.primary, fontWeight:800, marginTop:4 }}>📱 Unit: {assignedUnit.unit_code}</div>}
                        {req.note && <div style={{ fontSize:12, color:"#6b7280", fontStyle:"italic" }}>"{req.note}"</div>}
                        <div style={{ fontSize:11, color:"#9ca3af", marginTop:4 }}>{req.date}</div>
                      </div>
                      <StatusBadge s={req.status} />
        ...
        {tab === "report" && isStaff && (
          <div>
            ...
            <button onClick={() => { setModal("reportUnitIncident"); setForm({ reportedBy:currentUser.name }); }} style={{ ...btn, background:"linear-gradient(135deg,#c2410c,#f97316)", marginTop:0, marginBottom:20 }}>+ Report Damaged / Lost Item</button>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:200 }} onClick={() => setModal(null)}>
          <div style={{ background:"white", borderRadius:"20px 20px 0 0", padding:"24px 24px 32px", width:"100%", maxWidth:500, maxHeight:"85vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
            ...
            {modal === "request" && <>
              ...
              <p style={{ color:"#6b7280", fontSize:13, margin:"0 0 12px" }}>Your inventory assistant will assign the specific unit to you.</p>
              ...
              <input style={inp} value={form.note||""} onChange={e => setForm(f => ({ ...f, note:e.target.value }))} placeholder="e.g. For warehouse operations" />
              <button onClick={submitRequest} disabled={!form.itemId} style={{ ...btn, opacity:!form.itemId?0.5:1 }}>Submit Request</button>
            </>}
            ...
            {modal === "generateUnits" && <>
              ...
              <div style={{ background:BRAND.pale, borderRadius:10, padding:"10px 14px", marginTop:14, fontSize:13, color:BRAND.dark }}>
                Preview: <strong>{form.prefix||"XXX"}-{String(Number(form.startFrom)||1).padStart(3,"0")}</strong> to <strong>{form.prefix||"XXX"}-{String((Number(form.startFrom)||1)+(Number(form.count)||1)-1).padStart(3,"0")}</strong>
              </div>
