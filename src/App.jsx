...
const DarkModeStyles = () => (
  <style>{`
    :root {
      ...
      --modal-bg: #ffffff;
      --badge-pending-bg: #fef9c3; --badge-pending-txt: #a16207;
      --badge-approved-bg: #dcfce7; --badge-approved-txt: #15803d;
      --badge-rejected-bg: #fee2e2; --badge-rejected-txt: #b91c1c;
    }
    html.dark {
      --bg: #0f0f14;
      --surface: #1a1a2e;
...
export default function App() {
  ...
  // ── dynamic style helpers (dark-aware) ────────────────────────────────────
        ...
        <div style={{ background:"var(--surface)", borderRadius:20, padding:28, width:"100%", maxWidth:420, boxShadow:"0 25px 60px rgba(0,0,0,0.35)" }}>
          ...
          <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:8 }}>
            <button onClick={() => setDark(d => !d)} title="Toggle dark mode"
              style={{ background:"rgba(98,64,204,0.12)", border:"none", borderRadius:20, padding:"5px 12px", cursor:"pointer", fontSize:16, color:BRAND.primary, fontWeight:700 }}>
              {dark ? "☀️" : "🌙"}
            </button>
          </div>
      ...
      <div style={{ minHeight:"100vh", background:"var(--bg)", fontFamily:"system-ui,-apple-system,sans-serif" }}>
        ...
        <div style={{ background:BRAND.gradLight, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:50, boxShadow:"0 2px 12px rgba(98,64,204,0.4)" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            {/* Dark mode toggle */}
            <button onClick={() => setDark(d => !d)} title="Toggle dark mode"
              style={{ background:"rgba(255,255,255,0.18)", border:"none", color:"white", padding:"6px 10px", borderRadius:8, cursor:"pointer", fontSize:15, lineHeight:1 }}>
              {dark ? "☀️" : "🌙"}
            </button>
        ...
        <div style={{ maxWidth:680, margin:"0 auto", padding:"16px 12px 90px" }}>
          ...
          {tab==="requests" && isAdminOrAssistant && (
            <div>
              <h2 style={{ fontWeight:800, color:"var(--text)", marginBottom:16, fontSize:20 }}>Inventory Requests</h2>
              {requests.length===0 && <div style={{ color:"var(--text4)", textAlign:"center", marginTop:60 }}>No requests yet.</div>}
                  ...
                  <div key={status}>
                    ...
                    {group.map(t => {
                      ...
                      return (
                        <Card key={t.id}>
                          <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                            <div style={{ display:"flex", alignItems:"center", gap:4 }}><Avatar user={getUser(t.from_user_id)} size={32} /><span style={{ fontSize:14, color:"var(--text4)" }}>→</span><Avatar user={getUser(t.to_user_id)} size={32} /></div>
                            <div style={{ flex:1 }}>
                              ...
                              <div style={{ fontSize:14, fontWeight:700, marginTop:2, color:"var(--text)" }}>{unit?`📱 ${unit.unit_code}`:getItem(t.item_id)?.name}</div>
        ...
        {modal && (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:200 }} onClick={() => setModal(null)}>
            <div style={{ background:"var(--modal-bg)", borderRadius:"20px 20px 0 0", padding:"24px 24px 32px", width:"100%", maxWidth:500, maxHeight:"85vh", overflowY:"auto" }} onClick={e => e.stopPropagation()}>
              ...
              {/* TRANSFER */}
              {modal==="transfer" && <>
                <h3 style={{ margin:"0 0 4px", fontWeight:800, fontSize:18, color:"var(--text)" }}>Transfer Device</h3>
